...
class ListsSuite extends munit.FunSuite:
  ...
  test("sum of numbers with negatives included") {
    assertEquals(sum(List(-10,0,20)), 10)
  }

  test("sum of a list with one number") {
    assertEquals(sum(List(10)), 10)
  }
  ...
  test("list with negative numbers") {
    assertEquals(max(List(1,2,-1)), 2)
  }

  test("max of a list with one number") {
    assertEquals(max(List(10)), 10)
  }
